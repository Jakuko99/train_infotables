...
map = fo.Map(location=[48.40, 19.30], zoom_start=8, tiles="OpenStreetMap")
...
def save_map():
    while True:
        ...
        for vlak in response_obj:
            popup = fo.Popup(
                html=f"""<b>{vlak.get("DruhVlaku").strip()} {vlak.get("CisloVlaku")} {vlak.get("NazovVlaku")}</b> <a href="https://kis.zsr.sk/pis/popup.jsp?id=vlakDetail&vlkId={vlak.get('DetailVlakuStr')}" target="_blank"> Odkaz na PIS</a><br>
            ...
            <b>Dopravca:</b> {vlak.get("Dopravca")}""",
                max_width=400,
            )
            ...
            if vlak.get("Meskanie") < 20:
                fo.Marker(
                    location=vlak.get("Poloha"),
                    popup=popup,
                    tooltip=f'{vlak.get("DruhVlaku").strip()} {vlak.get("CisloVlaku")}',
            ...
            for part in trasa:
                fo.PolyLine(
                    locations=part.get("Value"),
                    color="red" if part.get("Key") == "Pred" else "blue",
                    weight=2.5,
                    opacity=1,
                ).add_to(map)
